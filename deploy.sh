...
echo "[4 de 4] ¿Deseas notificar a Google para reindexar la página? (s/n)"
read -r INDEXAR
if [ "$INDEXAR" = "s" ]; then
  if ! command -v jq &> /dev/null; then
    log "Error: jq no está instalado."
    echo "Error: jq no está instalado. Instálalo para continuar (por ejemplo: sudo apt install jq o brew install jq)"
    exit 1
  fi
  echo "Introduce la URL principal del sitio que deseas indexar (ejemplo: https://www.sinaloagaming.com/)"
  read -r URL_INDEXAR
  # Variables de entorno
  API_KEY=${GOOGLE_INDEXING_API_KEY:-"TU_API_KEY_AQUI"}
  JSON_KEY=${GOOGLE_INDEXING_JSON:-"ruta/a/tu/credencial.json"}
  if [ ! -f "$JSON_KEY" ]; then
    log "Error: No se encontró el archivo de credenciales JSON en $JSON_KEY."
    echo "Error: No se encontró el archivo de credenciales JSON en $JSON_KEY. Configura GOOGLE_INDEXING_JSON en tu .env."
    exit 1
  fi
  # Obtener token de acceso
  ACCESS_TOKEN=$(cat "$JSON_KEY" | \
    jq -r '.client_email as $email | .private_key as $key | {"alg":"RS256","typ":"JWT"} | @base64' | \
    tr -d '=' | tr '/+' '_-' | \
    xargs -I {} echo -n '{}')
...
EOF
  ...
  if [ "$ACCESS_TOKEN" = "null" ] || [ -z "$ACCESS_TOKEN" ]; then
    log "Error: No se pudo obtener el token de acceso de Google."
    echo "Error: No se pudo obtener el token de acceso. Revisa tu archivo de credenciales."
    exit 1
  fi
  ...
  RESPONSE=$(curl -s -X POST \
    ...
    -d '{"url": "'$URL_INDEXAR'", "type": "URL_UPDATED"}' \
    "https://indexing.googleapis.com/v3/urlNotifications:publish")
  log "Respuesta de Google Indexing API: $RESPONSE"
  echo "Respuesta de Google Indexing API: $RESPONSE"
else
  log "Saltando notificación a Google."
  echo "Saltando notificación a Google."
fi
